$(function () { // Makes sure that your function is called once all the DOM elements of the page are ready to be used.
  ...
  $('.log-info-button').click(logInfo);
  $('.log-warning-button').click(logWarning);
  $('.log-error-button').click(logError);
  $('.log-table-button').click(logTable);
  $('.log-group-button').click(logGroup);
  $('.log-custom-button').click(logCustom);

  $('.cause-404-button').click(cause404);
...
function logWarning() {
  console.warn("Warning boiiii");
}

function logError() {
  console.error("CLEAN UP ON ISLE 'MY CODE'");
}

function logTable() {
  console.table(Object.values(beastTypes), ["name", "weight", "happiness", "trust"]);
}

function logGroup() {
  console.group("Beast unshelter");
}

function logCustom() {
  console.log("mmmmm me likes this");
}


function cause404() {
  fetch("images/this-file-does-not-exist.png");
}

function causeTypeError() {
  var missingBeast = null;
  missingBeast.name = "Oops";
}
...
function causeViolation() {
  ...
  console.log("Violation test finished.");
}
